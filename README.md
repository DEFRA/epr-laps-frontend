...
[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=DEFRA_epr-laps-frontend&metric=security_rating)](https://sonarcloud.io/summary/new_code?id=DEFRA_epr-laps-frontend)
...
change the default value in `src/config/config.js`.
...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag epr-laps-frontend`

Build:

```bash
docker build --target development --no-cache --tag epr-laps-frontend:development .
```

Run:

```bash
docker run -p 3000:3000 epr-laps-frontend:development
```
...
```bash
docker build --no-cache --tag epr-laps-frontend .
```

Run:

```bash
docker run -p 3000:3000 epr-laps-frontend
```
